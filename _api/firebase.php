...
<script type="text/javascript" language="javascript" >
    function getKey () {
        return firebase.database().ref().child("teste").push().key; //chave
    }
    function update() {
        var obj = new Object();
        <?php $ultimapos = count($resultado)-1;
            echo "var matriz = [";
            foreach ($resultado as $key => $value) {
                echo "[";
                foreach ($value as $key2 => $value2) {
                    echo $tela = ($key2=="conclusao") ?  "'$value2'" : "'$value2',";
                }
                echo $fimMatriz = ($ultimapos==$key) ? "]" :  "]," ;
            }
            echo "];";
        ?>
        const final = matriz.length;
        console
        var matrizobj = [];
        for (let c=1;c<final;c++) {
            <?php
                for ($i=0; $i<$qtd_obj;$i++) { 
                    echo "obj." . $obj[$i] . "= matriz[c][" . $i . "];";
                }
            ?>
            matrizobj.push(obj);
        }
        <?php echo "return firebase.database().ref().child('$nome').set(matrizobj);"; ?>
        //firebase.database().ref().child($nome).push().set(matrizobj); chave unica
    }
    function list() {
...
<body id ="corpo"; onload='<?php echo 'return update();';?>' >
<div id="resultado">
    <script>
